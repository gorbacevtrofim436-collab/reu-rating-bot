...
- Мониторинг изменений: таблицы `rating_snapshots` и `schedule_snapshots` хранят последние снимки, а GitHub Actions раз в час будит Render и запускает проверку через защищенный endpoint. При смене недели бот отправляет полное расписание новой недели, а не список удаленных и добавленных пар.
...
REA_SCHEDULE_REQUEST_TIMEOUT=10
```
