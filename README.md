...
- Мониторинг изменений: таблица `rating_snapshots` хранит последний снимок баллов, фоновая задача проверяет изменения и отправляет уведомления.
...
MONITOR_INITIAL_DELAY_SECONDS=60
MONITOR_USER_DELAY_SECONDS=30
MONITOR_JITTER_SECONDS=60
```
...
Фоновые проверки пишут события `rating_monitor`, а найденные изменения пишутся как `rating_change`.
