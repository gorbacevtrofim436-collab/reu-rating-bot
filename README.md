...
- Мониторинг изменений: таблицы `rating_snapshots` и `schedule_snapshots` хранят последние снимки, а GitHub Actions раз в час будит Render и запускает проверку через защищенный endpoint.
...
Render Free может засыпать, поэтому встроенный background-loop выключен через `MONITOR_BACKGROUND_ENABLED=false`. Проверку запускает GitHub Actions workflow `.github/workflows/hourly-monitor.yml`, который раз в час делает `POST` на:
...
В обычном сценарии пользователь работает кнопками: `Баллы`, `Расписание пар`, `Уведомления`, `Удалить данные`, `Назад`. Для рейтинга бот показывает короткие кнопки предметов, например `матан`, `англ`, `алгоритмы`.
