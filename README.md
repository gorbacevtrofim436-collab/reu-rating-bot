...
- Точка входа: `bot.py`.
- Production на постоянно работающем VPS: polling через `RUN_MODE=polling`.
- Render Web Service: webhook через `aiohttp` HTTP-сервер.
...
- Мониторинг изменений: таблицы `rating_snapshots` и `schedule_snapshots` хранят последние снимки. На VPS отдельный systemd timer раз в час проверяет ЛКС РЭУ и отправляет уведомления. При смене недели бот отправляет полное расписание новой недели, а не список удаленных и добавленных пар.
- Конфигурация текущего VPS находится в [`deploy/README.md`](deploy/README.md). На VPS ежечасный мониторинг и резервные копии запускаются systemd timers.
...
- Администратор может отправить объявление всем пользователям, которые хотя бы раз нажали `/start`, через закрытую команду `/broadcast`.

## Routes для webhook-режима
...
## Ежечасный мониторинг через GitHub Actions для Render

Это резервный вариант для Render Free. Workflow `.github/workflows/hourly-monitor.yml` раз в час поднимает Python, подключается к Neon, проверяет ЛКС РЭУ и сам отправляет Telegram-уведомления. На текущем VPS workflow не нужен: вместо него включён systemd timer. В GitHub repo settings нужно добавить Actions secrets:
...
- `/broadcast` — подготовить объявление для всех подписчиков; доступно только пользователю из `BOT_ADMIN_USER_ID`.
